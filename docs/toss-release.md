...
검증: 자동 테스트 47개, 평가 사례 형식 검증 30개, 토스 패키징 통과. 기존 브라우저의 재시작 복원·신고·5턴 완주·설문·삭제 실패 검증에 더해, 답장 및 복습 응답 유실 복구·힌트 재사용·1턴 조기 종료·품질 제보 동의·복습 저장을 확인했습니다. 320/375/768/1440px에서 가로 넘침과 JavaScript 오류가 없었습니다. 실제 AI로 시간 경과 답장·조기 평가·복습 피드백 1사례를 확인했으며 추가 추정 비용은 $0.009108입니다. 토스 native back 콜백은 브라우저 모의 호출로 확인했으며 실제 기기 검증은 남아 있습니다.
...
`AI_BUDGET_USD=3`은 서버 전체 누적 한도이며 매일 자동 초기화되지 않습니다. 사용자별 기본 한도는 UTC 날짜 기준 AI 작업 100회입니다. AI 작업 시작 후 실패·차단도 횟수에 포함되지만, 캐시된 힌트·주제 조회와 잘못된 게임 입력, 완료한 요청 재전송은 포함되지 않습니다. 비용 상한 변경은 운영 예산을 정한 뒤 별도 검토합니다.

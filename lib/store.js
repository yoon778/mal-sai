...
export class Store {
  ...
  consumeAI(owner, dailyLimit, minuteLimit, now = Date.now(), sharedOwner) {
    const instant = new Date(now).toISOString();
    this.transaction(() => {
      for (const identity of sharedOwner ? [owner, sharedOwner] : [owner]) {
        try { this.consume(identity, dailyLimit, instant.slice(0, 10)); }
        catch (error) {
        ...
        try { this.consume(`minute:${identity}`, minuteLimit, instant.slice(0, 16)); }
        catch (error) {
          if (!(error instanceof GameError)) throw error;

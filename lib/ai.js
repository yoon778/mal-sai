...
출력 직전 확인: “당신”이라는 단어와 사용자에게 없는 이름을 절대 쓰지 않는다. factsAboutYou의 주체를 user로 뒤집지 않는다. roleRule, characterSpeechCue, styleCue, bubbleStyleCue를 어기지 않는다.`, {
        ...
        yourProfile: { age: game.profile.age, gender: game.profile.gender, speech: game.profile.speech, initiative: game.profile.initiative, humor: game.profile.humor },
      ...
      return normalizePartnerReply(result, game.profile.speech);
    },
    async hint(game) {
...
backgroundContext는 상황 이해에만 사용한다. 점수와 evidenceIds는 practiceTranscript의 user 메시지만 대상으로 한다.`, {
        ...
        practiceTranscript: messages.filter(message => !message.background),
      }, 2600, game.id);
    },
  };
}
...
export const demoAI = {
  ...
  async reply(game) {
    ...
    if (!active && game.turn === 2 && !reply.includes('?') && !/^(맞아요|저도|알겠어요)/.test(reply) && game.scenario.id !== 'cancelled') messages.unshift(game.profile.gender === 'female' ? '맞아요' : '저도요');
    ...
    return { messages: messages.map(message => naturalizeReply(message, game.profile.speech)), readAfterMinutes: game.scenario.id === 'cancelled' ? 30 : active ? 1 : 5, replyAfterReadMinutes: 1 };
  },

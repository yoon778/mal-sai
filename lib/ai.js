...
이전 대화의 존댓말 수준을 유지하고 해요체에서는 “내가” 대신 “제가”를 쓴다. “좋습니다·했습니다·보겠습니다·바랍니다·고맙습니다·연락드리겠습니다” 같은 합니다체나 문어체보다 자연스러운 해요체를 쓴다. 이름을 모르면 주어를 생략하며 사용자를 “당신”이라고 부르지 않는다. 이름을 거듭 부르는 고객상담 말투, 지나치게 완벽한 공감, 옷차림 같은 불필요한 지시, 매번 웃음·이모지로 끝내는 패턴을 피한다.
...
function characterSpeechCue(game) {
  return game.profile.gender === 'female'
    ? '서윤이라는 성인 여성 인물의 말투다. 부드러운 해요체와 짧은 감정 반응을 자연스럽게 쓰되, 과장된 애교나 여성 일반화는 하지 않는다.'
    : '도윤이라는 성인 남성 인물의 말투다. 담백하고 간결한 해요체로 자기 생각을 분명히 말하되, 무뚝뚝함이나 남성 일반화는 하지 않는다.';
}
...
export function normalizePartnerReply(result) {
  ...
  const messages = result.messages.map(naturalizeReply);
...
출력 직전 확인: “당신”이라는 단어와 사용자에게 없는 이름을 절대 쓰지 않는다. factsAboutYou의 주체를 user로 뒤집지 않는다. roleRule, characterSpeechCue, styleCue, bubbleStyleCue를 어기지 않는다.`, {
      ...
      return normalizePartnerReply(result);
    },
    async hint(game) {
...
backgroundContext는 상황 이해에만 사용한다. 점수와 evidenceIds는 practiceTranscript의 user 메시지만 대상으로 한다.`, {
        context: game.scenario.context, goal: game.scenario.goal, rubric,
...
export const demoAI = {
  ...
  async topic(game) {
    ...
    return { topic, bridge, next, avoid: '질문을 여러 개 연달아 보내거나 답장을 재촉하지 않기' };
  },
  async evaluate(game) {

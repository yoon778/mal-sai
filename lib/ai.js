...
const feedbackSchema = { type: 'array', items: object({ messageId: string, text: string }) };
const evaluationSchema = object({
  ...
  criteria: { type: 'array', items: object({
    ...
    score: { type: ['integer', 'null'], enum: [null, 0, 1, 2, 3, 4] }, reason: string, evidenceIds: strings,
  }) },
...
배경, 일정, 말투, 친밀도에 일관되게 반응한다. 성별에 따른 고정관념으로 행동하지 않는다. 아직 안 만났는데 만난 기억을 만들지 않는다.
...
humor light는 가벼운 농담을 즐기고 plain은 담백한 말투다. 과한 유행어, 상담체, 소설식 행동 묘사는 피한다.
  ...
  return {
    ...
    async reply(game) {
      const result = await generate('partner_reply', replySchema, counterpartPrompt, {
        scenario: game.scenario.context, facts: game.scenario.facts, profile: game.profile,
        turn: game.turn, startMinuteOfDay: game.scenario.startMinute, minute: game.minute, transcript: transcript(game),
      }, 800, game.id);
      if (!Array.isArray(result.messages) || result.messages.length < 1 || result.messages.length > 3 || result.messages.some(t => typeof t !== 'string' || !t.trim() || t.length > 400) || ![0, 1, 5, 15, 30, 60, 120].includes(result.readAfterMinutes) || ![0, 1, 5, 15, 30, 60, 120].includes(result.replyAfterReadMinutes)) throw new GameError('상대 응답을 확인하지 못했어요. 다시 시도해 주세요.', 502);
      return result;
    },
    async hint(game) {
    ...
    evaluate: game => generate('evaluation', evaluationSchema, evaluatorPrompt, {

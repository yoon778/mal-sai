...
const replySchema = object({
  messages: { type: 'array', items: shortString(400), minItems: 1, maxItems: 3 },
...
출력 직전 확인: “당신”이라는 단어와 사용자에게 없는 이름을 절대 쓰지 않는다. factsAboutYou의 주체를 user로 뒤집지 않는다. roleRule, characterSpeechCue, styleCue를 어기지 않는다.`, {
        ...
        characterSpeechCue: characterSpeechCue(game), styleCue: styleCue(game), turn: game.turn, startMinuteOfDay: game.scenario.startMinute, minute: game.minute, transcript: transcript(game),
      }, 800, game.id);
...
bridge는 현재 존댓말 수준을 유지하고 질문 공세가 되지 않게 쓴다. 숨겨진 정보나 성별 고정관념을 만들지 않는다.`, {

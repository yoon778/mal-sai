...
export function validateOperationSettings(env, { requireProxy = true } = {}) {
  ...
  if ((requireProxy || env.TRUSTED_PROXY_IP) && !isIP(env.TRUSTED_PROXY_IP ?? '')) throw new Error('TRUSTED_PROXY_IP must be one TLS reverse-proxy IP address');
  const limits = [

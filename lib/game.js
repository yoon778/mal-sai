...
function checkpoint(game) {
  return structuredClone({ minute: game.minute, messages: game.messages, hints: game.hints, topicHelps: game.topicHelps });
}

export function startGame(game) {
  if (game.stage !== 'preview') throw new GameError('이미 시작한 대화예요.');
  game.stage = 'chat';
  if (game.scenario.opener) game.messages.push({ id: randomUUID(), role: 'partner', text: game.profile.speech === 'casual' ? casualOpeners[game.scenario.id] : game.scenario.opener, minute: 0, readAt: null, turn: 0 });

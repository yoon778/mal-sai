...
import { coachingIssues } from './coaching.js';
...
export function publicGame(game) {
  return {
    ...
    revision: game.revision ?? 0, completedRequestIds: (game.requests ?? []).map(item => item.id),
    ...
    waiting: Boolean(game.pendingReply), totalHints: game.totalHints, hint: game.hints.find(item => item.turn === game.turn)?.text ?? null,
    totalTopicHelps: game.totalTopicHelps, topicHelp: game.topicHelps.find(item => item.turn === game.turn)?.help ?? null,
    finishedEarly: Boolean(game.finishedEarly), drill: game.drill ?? null,
...
export async function sendTurn(game, input, ai) {
  ...
  const messages = reply.messages.map(text => ({ id: randomUUID(), role: 'partner', text: chatText(text), minute: arrivesAt, readAt: null, turn: next.turn, aiVersion: next.aiVersions?.reply ?? null }));
...
export function normalizeEvaluation(raw, game) {
  ...
  const coachingVersion = raw.coachingVersion === 2 ? 2 : 1;
  if (coachingVersion === 2 && (!moments.length || coachingIssues(moments, game).length)) throw new GameError('답장 당시의 맥락과 개선 예시를 확인하지 못했어요 다시 시도해 주세요', 502);
  const coaching = moments.map(item => {
    if (!item || !own.has(item.messageId) || !['strength', 'improvement'].includes(item.kind)
      ...
      || !(coachingVersion === 2 && item.kind === 'strength' && item.alternative === null)
    ...
    return { messageId: item.messageId, kind: item.kind, reason: item.reason.trim(), alternative: item.alternative === null ? null : chatText(item.alternative), nextStep: item.nextStep.trim(), ...(coachingVersion === 2 ? { change: item.change.trim(), contextMessageIds: [...new Set(item.contextMessageIds)] } : {}), ...(principle ? { principle: { title: principle.title, source: principle.source } } : {}) };
  });
  ...
  return {
    ...
    moments: coaching.filter((item, index) => coaching.findIndex(other => coachingVersion === 2 ? own.get(other.messageId).turn === own.get(item.messageId).turn : other.messageId === item.messageId) === index), coachingVersion, provisional: true, mode: game.mode,
  };
}

export async function finishGame(game, ai, { early = false, refresh = false } = {}) {
  if (game.pendingReply) throw new GameError('마지막 답장을 기다린 뒤 복기할 수 있어요');
  if (refresh) {
    if (game.stage !== 'finished' || !game.result) throw new GameError('마친 대화에서만 복기를 다시 받을 수 있어요');
    if (game.result.coachingVersion === 2) return;
  } else {
    if (game.result) return;
    if (game.stage !== 'chat' || game.turn < 1 || game.turn > 5 || game.turn !== 5 && !early) throw new GameError('답장 5회를 마치거나 여기서 마무리하기를 선택해 주세요');
  }
  if (game.messages.some(m => m.role === 'partner' && m.readAt === null)) throw new GameError('마지막 메시지를 먼저 읽어주세요.');
  const evaluated = { ...game, finishedEarly: game.turn < 5 };
  game.result = normalizeEvaluation(await ai.evaluate(evaluated), evaluated);
  game.finishedEarly = evaluated.finishedEarly;
  game.aiVersions = evaluated.aiVersions ?? game.aiVersions;
  game.stage = 'finished';
}

export function retryTurn(game, turn) {
  if (game.stage !== 'finished' || !Number.isInteger(turn) || turn < 1 || turn > game.turn || !game.checkpoints[turn - 1]) throw new GameError('복기에서 다시 시작할 답장을 골라주세요.');
  ...
  Object.assign(game, restored, { turn: turn - 1, stage: 'chat', result: null, finishedEarly: false, drill: null });

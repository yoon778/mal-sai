...
function checkpoint(game) {
  return structuredClone({ minute: game.minute, messages: game.messages, hints: game.hints, topicHelps: game.topicHelps, pendingReply: game.pendingReply ?? null });
}

export function startGame(game) {
  if (game.stage !== 'preview') throw new GameError('이미 시작한 대화예요.');
  game.stage = 'chat';
  if (game.scenario.opener) game.messages.push({ id: randomUUID(), role: 'partner', text: chatText(game.profile.speech === 'casual' ? casualOpeners[game.scenario.id] : game.scenario.opener), minute: 0, readAt: null, turn: 0 });
...
export async function sendTurn(game, input, ai) {
  ...
  if (arrivesAt - next.minute >= 30) next.pendingReply = { arrivesAt, messages };
  else { next.minute = arrivesAt; next.messages.push(...messages); }
...
export function normalizeEvaluation(raw, game) {
  ...
  const coaching = moments.map(item => {
    ...
    return { messageId: item.messageId, kind: item.kind, reason: item.reason.trim(), alternative: chatText(item.alternative), nextStep: item.nextStep.trim() };
  });

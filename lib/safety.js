...
export function createSafety({ key = process.env.OPENAI_API_KEY, fetcher = fetch, stopFile } = {}) {
  return async text => {
    ...
    // Only an unambiguous standalone statement bypasses the contextual classifier.
    // Educational, quoted, and multi-message inputs still go through moderation.
    if (/^(?:(?:나|저)(?:는|도)?\s*)?(?:정말\s*)?(?:죽고\s*싶(?:어요|어|다)|자살(?:할래|하겠어|하고\s*싶어요)|자해(?:할래|하겠어|하고\s*싶어요))[.!?ㅠ\s]*$/u.test(text.trim())) throw new GameError('지금은 연습보다 안전이 먼저예요 혼자 견디지 말고 믿을 수 있는 사람이나 전문 상담기관에 바로 도움을 요청해 주세요', 422);
...
export function safeAI(ai, check) {
  ...
  for (const method of ['reply', 'hint', 'topic', 'evaluate', 'reviewDrill']) {
    ...
    wrapped[method] = async (...args) => {
      ...
      const game = args[0];
      await check(JSON.stringify({ recentConversation: game.messages?.slice(-8).map(m => ({ role: m.role, text: m.text })), ...(method === 'reviewDrill' ? { situation: game.drill, answer: args[1] } : {}), generatedResponse: result }));
      return result;
    };
  }
  return wrapped;

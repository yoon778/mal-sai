...
export function deployment(env = process.env) {
  ...
  if (!['local', 'toss', 'web'].includes(mode)) throw new Error('APP_PLATFORM must be local, toss or web');
  if (mode === 'local') {
    if (env.NODE_ENV === 'production') throw new Error('Production requires APP_PLATFORM=toss or web');
    return { mode };
  }
  if (mode === 'web') {
    const render = env.RENDER === 'true';
    const origin = httpsOrigin(env.API_PUBLIC_ORIGIN || (render ? env.RENDER_EXTERNAL_URL : undefined), 'API_PUBLIC_ORIGIN');
    validateOperationSettings(env, { requireProxy: !render });
    if (!/^[a-f\d]{64}$/i.test(env.STORAGE_KEY ?? '')) throw new Error('STORAGE_KEY is required for web');
    if (!['true', 'false'].includes(env.AI_ENABLED)) throw new Error('Web requires explicit AI_ENABLED=true or false');
    if (env.AI_ENABLED === 'true' && !env.OPENAI_API_KEY) throw new Error('AI_ENABLED=true requires OPENAI_API_KEY');
    return { mode, host: origin.host, origin: origin.origin, storageKey: env.STORAGE_KEY, proxy: render ? 'render' : 'fixed', trustedProxyIp: env.TRUSTED_PROXY_IP };
  }
...
export function createAccess(settings = { mode: 'local' }, verify = verifyTossKey, { verificationLimit = 300, sourceVerificationLimit = Math.min(30, verificationLimit), now = Date.now } = {}) {
  if (settings.mode === 'web') return createWebAccess(settings, { now });

...
export function replyExampleFor(game) {
  ...
  if (['joke-repair', 'clarify'].includes(id)) return casual
    ...
    : { user: '그런 뜻은 아니었어요 표현이 좀 이상했네요 미안해요', partner: ['아 그런 뜻이었군요'] };
  ...
  if (id === 'callback') return casual
    ? { user: '오늘 면접 어땠어?', partner: ['생각보다 괜찮았어', '끝나니까 배고프더라 ㅋㅋ'] }
    : { user: '오늘 면접 어땠어요?', partner: ['생각보다 괜찮았어요', '끝나니까 배고프더라고요 ㅋㅋ'] };
  return casual
    ? { user: '그 전시 어땠어?', partner: ['사진은 좋았어', '사람 많아서 오래 못 봤지만'] }
    : { user: '그 전시 어땠어요?', partner: ['사진은 좋았어요', '사람 많아서 오래 못 봤지만요'] };
}

...
function baseReplyExampleFor(game) {
...
export function replyExampleFor(game) {
  const base = baseReplyExampleFor(game);
  // Stable, authored variation: retries at the same turn keep the same reference.
  const variant = ((game.createdAt ?? 0) + game.turn) % 3;
  if (variant === 0) return base;
  const casual = game.profile.speech === 'casual';
  if (['joke-repair', 'clarify', 'busy-break', 'cancelled'].includes(game.scenario.id)) {
    return { ...base, partner: variant === 1 ? base.partner.map(text => text.replaceAll('\n', ' ')) : base.partner };
  }
  const examples = casual ? [
    { user: '그 책 다 읽었어?', partner: ['아직 반 정도\n생각보다 천천히 읽게 되네'] },
    { user: '비 많이 와?', partner: ['방금 그쳤어', '공기 시원하다'] },
  ] : [
    { user: '그 책 다 읽으셨어요?', partner: ['아직 반 정도요\n생각보다 천천히 읽게 되네요'] },
    { user: '비 많이 와요?', partner: ['방금 그쳤어요', '공기 시원하네요'] },
  ];
  return examples[variant - 1];
}

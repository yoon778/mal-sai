const scenarioPatterns = {
  ...
  cancelled: [
    ...
    [/몸조리\s*잘\s*하(?:세|시)|회복하(?:세|시)|건강\s*잘\s*챙기/, '사용자를 환자로 취급함'],
  ],
  'second-date': [
    [/토요일.{0,15}(비어|괜찮|가능)/, '토요일 약속과 모순됨'],
    [/일요일.{0,15}(약속(?:이|은)?\s*(?:있|잡)|바쁘|어려)/, '일요일 여유와 모순됨'],
  ],
...
export function assessRealismRun({ scenarioId, partnerName, partnerTurns, profile }) {
  ...
  for (const turn of partnerTurns) {
    ...
    if (profile.speech !== 'casual' && /(^|\s)내가(?:\s|$)/.test(text)) issues.push({ turn: turn.turn, type: 'style', text: '존댓말 흐름에서 반말 1인칭 사용' });
    if (/좋습니다|했습니다|보겠습니다|바랍니다|고맙습니다|감사합니다|연락드리겠습니다/.test(text)) issues.push({ turn: turn.turn, type: 'style', text: '메신저 대화보다 딱딱한 문어체' });
  ...
  const multiBubbleTurns = partnerTurns.filter(turn => turn.messages.length > 1).length;
  const tripleBubbleTurns = partnerTurns.filter(turn => turn.messages.length === 3).length;
  const microBubbleTurns = partnerTurns.filter(turn => turn.messages.some(message => message.trim().length <= 12)).length;
  for (const turn of partnerTurns) {
    if (turn.messages.length === 3 && turn.messages.every(message => message.trim().length <= 3)) issues.push({ turn: turn.turn, type: 'segmentation', text: '짧은 조각만으로 기계적으로 분절됨' });
    if (turn.messages.some((message, index) => index > 0 && message.trim() === turn.messages[index - 1].trim())) issues.push({ turn: turn.turn, type: 'segmentation', text: '같은 말풍선이 연속 반복됨' });
  }
  // Frequencies are diagnostic metrics, not quotas imposed on natural dialogue.
  return {
    ...
    playfulTurns, multiBubbleTurns, tripleBubbleTurns, microBubbleTurns,
...
export function summarizeRealism(results, expectedRuns = 10) {
  ...
  const segmentationIssues = issues.filter(issue => issue.type === 'segmentation').length;
  ...
  const bubbleCount = results.reduce((sum, result) => sum + (result.bubbleCount ?? 0), 0);
  const multiBubbleTurns = results.reduce((sum, result) => sum + (result.multiBubbleTurns ?? 0), 0);
  const tripleBubbleTurns = results.reduce((sum, result) => sum + (result.tripleBubbleTurns ?? 0), 0);
  const microBubbleTurns = results.reduce((sum, result) => sum + (result.microBubbleTurns ?? 0), 0);
  const averageBubblesPerTurn = partnerTurns ? bubbleCount / partnerTurns : 0;
  const multiBubbleRate = partnerTurns ? multiBubbleTurns / partnerTurns : 0;
  const tripleBubbleRate = partnerTurns ? tripleBubbleTurns / partnerTurns : 0;
  const microBubbleRate = partnerTurns ? microBubbleTurns / partnerTurns : 0;
  const segmentationPass = expectedRuns < 10 || averageBubblesPerTurn >= 1.2 && averageBubblesPerTurn <= 2.6 && tripleBubbleRate <= 0.35;
  return {
    runs: results.length, partnerTurns, bubbleCount, roleConfusions, factContradictions, styleIssues, segmentationIssues, repetitionRate,
    activeQuestionRate, calmQuestionRate, lightPlayfulRate, plainPlayfulRate, personalityPass,
    averageBubblesPerTurn, multiBubbleRate, tripleBubbleRate, microBubbleRate, segmentationPass,
    pass: results.length === expectedRuns && partnerTurns === expectedRuns * 5 && roleConfusions === 0 && factContradictions === 0 && styleIssues === 0 && segmentationIssues === 0 && repetitionRate < 0.3,
  };
}

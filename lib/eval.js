...
export function validateCases(document, researchMarkdown) {
  ...
  const scope = Array.isArray(document?.scenarioIds) ? document.scenarioIds : [];
  if (!Array.isArray(scope) || scope.length !== 5 || new Set(scope).size !== scope.length || scope.some(id => !knownScenarios.has(id))) errors.push('기존 평가 기준의 상황 5개를 명시해야 함');
  ...
  for (const item of cases) {
    ...
    if (!scope?.includes(item?.scenarioId)) errors.push(`${where}: 명시한 평가 범위 밖의 상황`);
  ...
  for (const scenario of scenarios.filter(item => scope?.includes(item.id))) {
...
export function buildEvalGame(item, { gender = 'female' } = {}) {
  ...
  const profile = { name: gender === 'male' ? '도윤' : '서윤', gender, age: 27, speechStyle: gender === 'male' ? '담백한 해요체' : '부드러운 해요체', initiative: 'calm', humor: 'plain' };
  return {

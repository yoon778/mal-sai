...
import { practiceScenarios } from './practice-scenarios.js';
...
export const scenarios = [
  {
    ...
    facts: '나는 영화와 동네 산책을 좋아한다. 첫날 업무 종료 예정 시각은 21시다. 다음 만남은 대화에서 실제 합의한 내용만 따른다.',
    roleRule: '업무가 남아 있던 사람은 나이고 사용자는 아니다. 현재 시각과 일정 상태는 virtualClock을 따른다. 사용자가 내 바쁨을 배려하면 사용자에게 고생하세요라고 주체를 뒤집지 않는다.',
  ...
  },
  {
    ...
    roleRule: '내가 감기 몸살 때문에 취소했다. 사용자가 아프다는 정보는 없다. 사용자의 걱정에는 나의 휴식이나 고마움으로 답한다. 같은 걱정이 반복되면 연락·회복 약속을 되풀이하지 않는다. 내 증상을 사용자의 증상으로 뒤집지 않는다.',
...
scenarios.push(...practiceScenarios);
...
export function backgroundFor(scenario, profile) {
  const background = profile.speech === 'casual' ? (scenario.casualBackground ?? casualBackgrounds[scenario.id]) : scenario.background;
  return background.map((text, i) => ({
    id: `b${i}`, role: i % 2 ? 'partner' : 'user', background: true, turn: 0,
    text: i % 2 ? (profile.humor === 'light' && i === scenario.humorCue.index ? (profile.speech === 'casual' ? (scenario.casualHumor ?? casualHumorCues[scenario.id]) : scenario.humorCue.text) : text.replace(/ ㅎㅎ/g, profile.humor === 'light' ? ' ㅎㅎ' : '')) + (i === 7 && profile.initiative === 'active' && scenario.activeEnding ? ` ${profile.speech === 'casual' ? casualActiveEndings[scenario.id] : scenario.activeEnding}` : '') : text,

export const scenarios = [
  {
    ...
    facts: '상대는 영화와 동네 산책을 좋아한다. 오늘 9시까지 업무가 남아 있다. 다음 만남은 아직 정하지 않았다.',
  ...
  },
  {
    ...
    facts: '서로 독서 모임에서 처음 만났다. 상대는 에세이를 좋아하며 주말에 동네 서점에 간다. 아직 단둘이 만난 적 없다.',
  ...
  },
  {
    ...
    facts: '상대는 이번 주 프로젝트 마감 때문에 바빴다. 대화는 싸움 없이 끝났다. 이틀간 연락이 없었다는 사실만으로 호감은 알 수 없다.',
  ...
  },
  {
    ...
    facts: '소개팅 이후 일주일간 연락했다. 둘 다 카페를 좋아한다. 상대는 이번 토요일 약속이 있고 일요일 오후는 비어 있다. 일정은 대화로 확인할 수 있다.',
  ...
  },
  {
    ...
    facts: '오늘 두 번째 만남이 예정되어 있었다. 상대는 감기 몸살로 취소했다. 회복 시점은 아직 모른다. 취소가 곧 무관심을 의미하지 않는다.',
...
export const rubric = [
  { id: 'context', label: '맥락 이어가기', weight: 25, description: '상대가 말한 관심사·상황을 이해하고 연결했는가' },
  { id: 'reciprocity', label: '서로 주고받기', weight: 20, description: '질문과 자기 이야기가 상대에게 응답할 여지를 주는가' },
  ...
  { id: 'respect', label: '속도와 배려', weight: 20, description: '상대 일정·거절·경계를 존중했는가. 특정 지연 시간 자체는 감점하지 않는다' },
];
...
export function backgroundFor(scenario, profile) {
  return scenario.background.map((text, i) => ({
    id: `b${i}`, role: i % 2 ? 'partner' : 'user', background: true, turn: 0,
    text: i % 2 ? (profile.humor === 'light' && i === scenario.humorCue.index ? scenario.humorCue.text : text.replace(/ ㅎㅎ/g, profile.humor === 'light' ? ' ㅎㅎ' : '')) + (i === 7 && profile.initiative === 'active' ? ` ${scenario.activeEnding}` : '') : text,
